...
@pytest.fixture
def store():
    """In-memory MetricsStore pre-loaded with 48 hourly observations."""
    s = MetricsStore()
    for h in range(48):
        if h < 24:
    ...
    s.record_event("deployment", "v1.0 deployed",
                   {"version": "v1.0"}, "2024-01-01 00:00:00")
    s.record_event("ab_start", "A/B test started",
                   {"treatment_pct": 10}, "2024-01-01 12:00:00")
    return s
...
class TestMetricsStore:
    ...
    def test_metric_count(self, store):
        # 48 time points × 2 variants × 2 metrics = 192
    ...
    def test_bulk_record_metrics(self):
        s = MetricsStore()
        rows = [
            ("v1", "control",   "ctr", 0.20 + i * 0.01, f"2024-06-01 {i:02d}:00:00")
            for i in range(10)
        ]
    ...
    def test_query_window_correct_range(self, store):
        # 24 hours on day 1 × 2 variants = 48 rows
    ...
    def test_downsample_daily_row_count(self, store):
        daily = store.downsample(
            "precision_at_10",
            "%Y-%m-%d",
            "2024-01-01 00:00:00",
            "2024-01-02 23:00:00",
        )
        # 2 days × 2 variants = 4 rows
        assert len(daily) == 4
    ...
    def test_downsample_is_less_than_raw(self, store):
        raw   = store.query_window("precision_at_10",
                                   "2024-01-01 00:00:00", "2024-01-02 23:00:00")
        daily = store.downsample("precision_at_10", "%Y-%m-%d",
                                 "2024-01-01 00:00:00", "2024-01-02 23:00:00")
    ...
    def test_detect_trend_improving(self):
        ...
        trend = s.detect_trend("prec", "control", window_size=10)
    ...
    def test_detect_trend_insufficient_data_is_stable(self):
        ...
        trend = s.detect_trend("prec", "control", window_size=5)
    ...
    def test_query_events_respects_time_boundary(self, store):
        # Only the deployment event (00:00) falls before noon; ab_start is at 12:00
